...
# Mock CSEP classes for compatibility
class CSEPCatalog:
...
def build_quadtree_recursive(node, lons, lats, max_depth=4, min_events=10, target_bins=24):
    """Build quadtree recursively until target_bins reached or constraints met."""
    idx = np.where(
    ...
    # Stop if we've reached the target number of bins
    if node.depth >= max_depth or len(idx) <= min_events:
        return

    # More aggressive subdivision to reach target bins
    # Calculate how many bins we would have if we subdivided this node
    potential_bins = 4 ** (max_depth - node.depth)
    
    # Always subdivide if we're far from target
    if len(idx) > min_events * 2:  # Only stop if we have very few events
        node.subdivide()
        for child in node.children:
            build_quadtree_recursive(child, lons, lats, max_depth, min_events, target_bins)
...
def apply_quadtree_binning(catalog, max_depth=4, min_events=10, merge_threshold=None, max_bin_size=10.0, target_bins=24):
    ...
    print(f"🌳 Building quadtree with max_depth={max_depth}, min_events={min_events}, target_bins={target_bins}")

    root = QuadtreeNode((min_lon, max_lon, min_lat, max_lat))
    build_quadtree_recursive(root, lons, lats, max_depth, min_events, target_bins)
    ...
    # Adjust to exactly target_bins
    if len(unmerged_bounds) != target_bins:
        print(f"🎯 Adjusting from {len(unmerged_bounds)} to exactly {target_bins} bins...")
        if len(unmerged_bounds) > target_bins:
            # Need to merge some bins
            merge_count = len(unmerged_bounds) - target_bins
            print(f"  🔄 Merging {merge_count} bins to reach target...")
            unmerged_bounds = merge_to_target_bins(unmerged_bounds, lons, lats, target_bins, max_bin_size)
        else:
            # Need to split some bins
            split_count = target_bins - len(unmerged_bounds)
            print(f"  ✂️  Splitting {split_count} bins to reach target...")
            unmerged_bounds = split_to_target_bins(unmerged_bounds, lons, lats, target_bins, max_bin_size)
...
class QuadtreeBinner:
    ...
    def __init__(self, max_depth: int = 4, min_events: int = 10, merge_threshold: int = 50, max_bin_size: float = 12.0, 
                 custom_bounds: Tuple[float, float, float, float] = None, target_bins: int = 24):
    ...
    def assign_bins(self, lats: np.ndarray, lons: np.ndarray) -> np.ndarray:
        ...
        if self.custom_bounds:
            ...
            root = QuadtreeNode((min_lon, max_lon, min_lat, max_lat))
            build_quadtree_recursive(root, filtered_lons, filtered_lats, self.max_depth, self.min_events, self.target_bins)
            unmerged_bounds = extract_leaf_bounds(root)
            ...
            root = QuadtreeNode((min_lon, max_lon, min_lat, max_lat))
            build_quadtree_recursive(root, lons, lats, self.max_depth, self.min_events, self.target_bins)
        ...
        # Adjust to exactly target_bins
        if len(unmerged_bounds) != self.target_bins:
            print(f"🎯 Adjusting from {len(unmerged_bounds)} to exactly {self.target_bins} bins...")
            if len(unmerged_bounds) > self.target_bins:
                # Need to merge some bins
                merge_count = len(unmerged_bounds) - self.target_bins
                print(f"  🔄 Merging {merge_count} bins to reach target...")
                unmerged_bounds = merge_to_target_bins(unmerged_bounds, filtered_lons, filtered_lats, self.target_bins, self.max_bin_size)
            else:
                # Need to split some bins
                split_count = self.target_bins - len(unmerged_bounds)
                print(f"  ✂️  Splitting {split_count} bins to reach target...")
                unmerged_bounds = split_to_target_bins(unmerged_bounds, filtered_lons, filtered_lats, self.target_bins, self.max_bin_size)
        
        # Force merge to exactly target_bins if we're still over
        if len(unmerged_bounds) > self.target_bins:
            print(f"\n🎯 Forcing merge to exactly {self.target_bins} bins...")
            print(f"Current: {len(unmerged_bounds)} bins")
            
            # Use aggressive merging to reach target
            merged_bounds = merge_to_target_bins(
                unmerged_bounds, filtered_lons, filtered_lats, self.target_bins, self.max_bin_size
            )
            
            print(f"After forced merging: {len(merged_bounds)} bins")
            unmerged_bounds = merged_bounds
...
def merge_to_target_bins(bounds, lons, lats, target_bins, max_bin_size):
    """Merge bins to reach exactly target_bins."""
    if len(bounds) <= target_bins:
        return bounds
    
    # Sort bins by event count (merge lowest count bins first)
    bin_counts = {b: count_events_in_bin(b, lons, lats) for b in bounds}
    sorted_bounds = sorted(bounds, key=lambda b: bin_counts[b])
    
    # Calculate how many merges we need
    merges_needed = len(bounds) - target_bins
    
    working_bounds = bounds.copy()
    merges_performed = 0
    max_merges = merges_needed * 3  # Allow more attempts
    
    while len(working_bounds) > target_bins and merges_performed < max_merges:
        merges_performed += 1
        
        # Find the lowest count bin and its best neighbor
        if not sorted_bounds:
            break
            
        current_bin = sorted_bounds[0]
        if current_bin not in working_bounds:
            sorted_bounds.pop(0)
            continue
        ...
        for other_bin in working_bounds:
            if other_bin == current_bin:
                continue
                
            if are_bins_adjacent(current_bin, other_bin):
                # Score based on count (prefer merging with lower count bins)
                other_count = bin_counts[other_bin]
                score = other_count
        ...
        if best_neighbor is not None:
            # Create merged bin
            merged_bin = (
                min(current_bin[0], best_neighbor[0]), max(current_bin[1], best_neighbor[1]),
                min(current_bin[2], best_neighbor[2]), max(current_bin[3], best_neighbor[3])
            )
            ...
            # Allow larger bins if we're far from target
            effective_max_size = max_bin_size * 1.5 if len(working_bounds) > target_bins * 1.5 else max_bin_size
            
            if merged_width <= effective_max_size and merged_height <= effective_max_size:
                # Remove both original bins and add merged bin
                working_bounds.remove(current_bin)
                working_bounds.remove(best_neighbor)
                working_bounds.append(merged_bin)
                
                # Update bin counts
                new_count = bin_counts[current_bin] + bin_counts[best_neighbor]
                bin_counts[merged_bin] = new_count
                
                # Remove from sorted list
                sorted_bounds.pop(0)
                
                print(f"    ✅ Merged bins: {current_bin} + {best_neighbor} → {merged_bin} ({new_count} events)")
            else:
                # Skip this bin and try the next one
                sorted_bounds.pop(0)
        else:
            # No neighbor found, skip this bin
            sorted_bounds.pop(0)
    
    print(f"    🎯 Merged {len(bounds)} → {len(working_bounds)} bins (target: {target_bins})")
    return working_bounds


def split_to_target_bins(bounds, lons, lats, target_bins, max_bin_size):
    """Split bins to reach exactly target_bins."""
    if len(bounds) >= target_bins:
        return bounds
    
    # Calculate how many splits we need
    splits_needed = target_bins - len(bounds)
    
    working_bounds = bounds.copy()
    
    # Sort bins by event count (split highest count bins first)
    bin_counts = {b: count_events_in_bin(b, lons, lats) for b in bounds}
    sorted_bounds = sorted(bounds, key=lambda b: bin_counts[b], reverse=True)
    
    splits_performed = 0
    max_splits = min(splits_needed * 2, 100)  # Prevent infinite loops
    
    while len(working_bounds) < target_bins and splits_performed < max_splits:
        splits_performed += 1
        
        # Find the highest count bin to split
        if not sorted_bounds:
            break
            
        current_bin = sorted_bounds[0]  # Always take the first one
        if current_bin not in working_bounds:
            sorted_bounds.pop(0)  # Remove from sorted list
            continue
            
        # Split the bin into 4 smaller bins
        min_lon, max_lon, min_lat, max_lat = current_bin
        mid_lon = (min_lon + max_lon) / 2
        mid_lat = (min_lat + max_lat) / 2
        
        # Check if splitting would create bins that are too small
        width = max_lon - min_lon
        height = max_lat - min_lat
        
        if width < 0.5 or height < 0.5:  # Minimum bin size
            sorted_bounds.pop(0)  # Remove from sorted list
            continue
            
        # Create 4 smaller bins
        new_bins = [
        ...
        # Remove the original bin and add the new ones
        working_bounds.remove(current_bin)
        working_bounds.extend(new_bins)
        
        # Remove from sorted list
        sorted_bounds.pop(0)
        
        print(f"    ✂️  Split bin {current_bin} into 4 smaller bins")
        
        # Check if we've reached the target
        if len(working_bounds) >= target_bins:
            break
    
    print(f"    🎯 Final bin count: {len(working_bounds)} (target: {target_bins})")
    return working_bounds
